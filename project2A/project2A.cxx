		...
		RemoveVTKOpenGLStateSideEffects();
			...
			if (initialized == false) {
				...
				for (int i = 0; i < triangles.size(); i++)
				{
					...
					{
						//float ambient[3];
						//ambient[0] = triangles[i].normals[j][0];
						//ambient[1] = triangles[i].normals[j][1];
						//ambient[2] = triangles[i].normals[j][2];
						//glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, ambient);
						//glColor3ubv(colorMap[3 * (int)(triangles[i].fieldValue[j] * 25) + 0], colorMap[3 * (int)(triangles[i].fieldValue[j] * 25) + 1], colorMap[3 * (int)(triangles[i].fieldValue[j] * 25) + 2]);
		...
		virtual void RenderPiece(vtkRenderer *ren, vtkActor *act)
		{
			...
			if (initialized == false) {
				...
				for (int i = 0; i < triangles.size(); i++)
				{
					...
					//	int cIndex = (int) (triangles[i].fieldValue[j] * 256);
					//	for(int k = 0; k < 3; k++) {
					//		Texture3[j * 3 + k] = colorMap[3 * cIndex + k];
					//	}
					//}
